...
zaku.prototype = {
  ...
  zakuInit()
  {
      ...
      this.realCoords = this.SVG.createSVGPoint();

  },
	
  // initEvents(zakuInstance)
  initEvents()
  {
     var zakuInstance = this;
     var self = this;
     this.SVG.addEventListener('load', function(evt){

        self.__initEvents(zakuInstance);
  ...
  __initEvents(zakuInstance)
  {
    ...
    this.SVG.addEventListener('mousedown', function(evt){
        ...
        self.zakuDragStart(evt);
 
    })    
    //Call zakuDrag 
   this.SVG.addEventListener('mousemove', function(evt){
      
    self.zakuDrag(evt, zakuInstance);
  ...
  GetRealCoords(evt)
  {
   ...
   var newScale = this.SVG.currentScale;
   var translation = this.SVG.currentTranslate;
   this.realCoords.x = (evt.clientX - translation.x)/newScale;
   this.realCoords.y = (evt.clientY - translation.y)/newScale;
  ...
  zakuDragStart(evt)
  {
      ...
      var elemClasses = evt.target.classList;
      if(elemClasses.contains('zaku-move-point'))
      {
  ...
  zakuDrag(evt, zakuInstance)
  {
     ...
     if(this.target)
     {
        ...
        this.GetRealCoords(evt);

        this.target.setAttribute('cx',this.realCoords.x)
        this.target.setAttribute('cy',this.realCoords.y)


        var movePointX = this.realCoords.x;
        var movePointY = this.realCoords.y;

        var  newX = 0; //Set to 0 :Cause we don't have external changes such transform ...
        var  newY = 0;

        var movePointType  = this.target.getAttribute('type');
        var movePointNature = this.target.getAttribute('nature');
        var pathArrayIndex = this.target.getAttribute('pathArrayIndex');
        var pointIndex = this.target.getAttribute('pointIndex');

        //////////UPDATES///////////////
            /*Monoval Updates*/
            if(zakuInstance.isMonoVal(movePointType))
             {
                    var mpoints = zakuInstance.getMonoValPoint(zakuInstance.pathArray, pathArrayIndex , movePointType);
                    var newpathArray = zakuInstance.setMonoValPoint(zakuInstance.pathArray, pathArrayIndex, pointIndex, movePointType ,mpoints, newX, newY, movePointX, movePointY)
                    zakuInstance.update(zakuInstance.activeElem , newpathArray)

             }

            /*Bival Updates*/
            if(zakuInstance.isBiVal(movePointType))
             {
                  var mpoints = zakuInstance.getBiValPoint(zakuInstance.pathArray, pathArrayIndex , movePointType);
                  /*Updating point and pathArray*/
                  var newpathArray = zakuInstance.setBiValPoint(zakuInstance.pathArray, pathArrayIndex, pointIndex, movePointType ,mpoints, newX, newY, movePointX, movePointY)
                  zakuInstance.update(zakuInstance.activeElem , newpathArray)

             }
            
            /*Quadrival Updates*/
            if(zakuInstance.isQuadriVal(movePointType))
             {
                     
                    var mpoints = zakuInstance.getQuadriValPoint(zakuInstance.pathArray, pathArrayIndex , movePointType);
                    /*Updating point and pathArray*/
                    var newpathArray = zakuInstance.setQuadriValPoint(zakuInstance.pathArray, pathArrayIndex, pointIndex, movePointType ,mpoints, newX, newY, movePointX, movePointY)
                    zakuInstance.update(zakuInstance.activeElem , newpathArray)

             }

            /*Sixval Updates*/
            if(zakuInstance.isSixVal(movePointType))
             {
                     
                    var mpoints = zakuInstance.getSixValPoint(zakuInstance.pathArray, pathArrayIndex , movePointType);
                    /*Updating point and pathArray*/
                    var newpathArray = zakuInstance.setSixValPoint(zakuInstance.pathArray, pathArrayIndex, pointIndex, movePointType ,mpoints, newX, newY, movePointX, movePointY)
                    zakuInstance.update(zakuInstance.activeElem , newpathArray)

             }

           /*Updating points positions*/
           zakuInstance.updatepoints(zakuInstance.activeElem, zakuInstance.pathArray);
  ...
  split:function(path)
  {
      ...
      //Setting instance's pathArray
      this.pathArray = newpathArray;
  ...
  createLinkPath:function([mx,m],[lx,ly])
  {
        var dataPath = 'M'+mx+','+my+' L'+lx+','+ly;
        var path = document.createElementNS("http://www.w3.org/2000/svg", 'path');
        path.setAttribute('class','zaku-link-path zaku-ui-item');
        path.setAttribute('d', dataPath);
        path.setAttribute('stroke', '#000');
        path.setAttribute('stroke-width', '0.5');
        path.setAttribute('style', 'position:relative;z-index:9998');
    ...
    var zakuID = this.setzakuID();

		var point = document.createElementNS("http://www.w3.org/2000/svg", 'circle');
    point.setAttribute('id', "zaku-point"+zakuID);
		point.setAttribute('class','zaku-move-point zaku-ui-item');
    ...
    point.setAttribute('nature', natureTemp);
    point.setAttribute('pathArrayIndex', indexInPathArray);
    point.setAttribute('pointIndex', pointIndex);
		point.setAttribute('style', 'position:relative;z-index:9999');

		this.SVG.appendChild(point);

		 
    //Getting previous and create path element link 
    var zakuIDPrev = zakuID - 1;
    var prev_zakuID = "zaku-point"+(zakuIDPrev);

    var prevPoint = this.SVG.getElementById(prev_zakuID);
   

    if (prevPoint)
     {
         var pointNature = prevPoint.getAttribute('nature');
       
         if (pointNature === 'b')
          {
            //Getting path data params
            mx = parseFloat(prevPoint.getAttribute('cx'),10) + 0; 
            my = parseFloat(prevPoint.getAttribute('cy'),10) + 0; 

            lx = parseFloat(point.getAttribute('cx'),10) + 0 ; 
            ly = parseFloat(point.getAttribute('cy'),10) + 0 ; 
            
            var pointsConnector = this.createLinkPath([mx,my], [lx,ly]);

            this.SVG.appendChild(pointsConnector);
  ...
  removepoints:function()
  {
     /**********Clear all movePoints************/
       ...
       var svg = document.getElementsByTagName('svg')[0];
       var movePoints = document.getElementsByClassName('zaku-ui-item');
  ...
  // setpoints:function(elem, pathArray)
	setpoints:function()
	{
       var elem = zaku.activeElem, pathArray = zaku.pathArray;
